...
class CleverKitchenDatabase(context:Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    override fun onCreate(db: SQLiteDatabase) {
        ...
        db.execSQL("CREATE TABLE ${USER_DETAILS_TABLE}(${COL_EMAIL_ID} TEXT PRIMARY KEY ,$COL_FIRST_NAME TEXT,$COL_LAST_NAME TEXT, $COL_PHONE TEXT, $COL_USER_NAME TEXT, " +
                "$COL_PASSWORD TEXT)")
    ...
    fun insertUser(email:String?,firstName:String?,lastName:String?,phone:String?,username: String?, password: String?): Boolean {
        val sqliteDatabase = this.writableDatabase
        val contentValues = ContentValues()
        contentValues.put(COL_EMAIL_ID,email)
